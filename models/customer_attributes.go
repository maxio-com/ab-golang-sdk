...
func (c CustomerAttributes) String() string {
    return fmt.Sprintf(
    	"CustomerAttributes[FirstName=%v, LastName=%v, Email=%v, CcEmails=%v, Organization=%v, Reference=%v, Address=%v, Address2=%v, City=%v, State=%v, Zip=%v, Country=%v, Phone=%v, Verified=%v, TaxExempt=%v, Surcharging=%v, VatNumber=%v, Metafields=%v, ParentId=%v, SalesforceId=%v, DefaultAutoRenewalProfileId=%v, AdditionalProperties=%v]",
    	c.FirstName, c.LastName, c.Email, c.CcEmails, c.Organization, c.Reference, c.Address, c.Address2, c.City, c.State, c.Zip, c.Country, c.Phone, c.Verified, c.TaxExempt, c.Surcharging, c.VatNumber, c.Metafields, c.ParentId, c.SalesforceId, c.DefaultAutoRenewalProfileId, c.AdditionalProperties)
}
...
func (c CustomerAttributes) MarshalJSON() (
    ...
    if err := DetectConflictingProperties(c.AdditionalProperties,
        "first_name", "last_name", "email", "cc_emails", "organization", "reference", "address", "address_2", "city", "state", "zip", "country", "phone", "verified", "tax_exempt", "surcharging", "vat_number", "metafields", "parent_id", "salesforce_id", "default_auto_renewal_profile_id"); err != nil {
        return []byte{}, err
    }
...
func (c *CustomerAttributes) UnmarshalJSON(input []byte) error {
    ...
    additionalProperties, err := ExtractAdditionalProperties[interface{}](input, "first_name", "last_name", "email", "cc_emails", "organization", "reference", "address", "address_2", "city", "state", "zip", "country", "phone", "verified", "tax_exempt", "surcharging", "vat_number", "metafields", "parent_id", "salesforce_id", "default_auto_renewal_profile_id")
    if err != nil {
    	return err
    }
