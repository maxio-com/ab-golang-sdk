...
// returns an models.ApiResponse with []models.CouponCurrency data and
// an error if there was an issue with the request or response.
...
func (c *CouponsController) UpdateCouponCurrencyPrices(
	...
	req := c.prepareRequest(
		ctx,
		"PUT",
		fmt.Sprintf("/coupon/%v/currency_prices.json", couponId),
	)
	req.Authenticate(true)
	req.Header("Content-Type", "application/json")
	if body != nil {
	...
	var result []models.CouponCurrency
	decoder, resp, err := req.CallAsJson()
	if err != nil {
		return models.NewApiResponse(result, resp), err
	}
	...
	result, err = utilities.DecodeResults[[]models.CouponCurrency](decoder)
	if err != nil {
		return models.NewApiResponse(result, resp), err
	}
